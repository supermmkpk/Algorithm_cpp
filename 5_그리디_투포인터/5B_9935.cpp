 ...
 * <입력>
 * 1_문자열_누적합_구현) 문자열 (1_문자열_누적합_구현<= 길이 <= 1e6)
 * 2_Graph_DFS_BFS) 폭발 문자열 (1_문자열_누적합_구현<= 길이 <= 36)
 * 문자열 모두 알파벳 소문자와 대문자, 숫자 0~9로만 이루어져 있다.
 * <출력> 폭발이 끝난 후 남은 문자열 (남아있는 문자가 없는 경우, "FRULA")
 */

/**
 * 1_문자열_누적합_구현) 무식하게 이중 for문 : 100만 * 100만 : 너무 큼!
 * 2_Graph_DFS_BFS) for문 하나로 : 뒤에서 폭발문자열 만큼 확인하고 제거하기!
