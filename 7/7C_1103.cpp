 ...
 *  1. 동전이 있는 곳의 숫자 X에 대하여
 *  2. 위, 아래, 왼, 오른 방향 중 1개 선
 *  3. 선택한 방향으로 X만큼 움직인다. (중간에 있는 구멍은 무시)
 * 동전이 구멍에 빠지거나, 보드의 바깥으로 나간다면 게임 종료.
 * 최대 몇 번 동전을 움직일 수 있는지 구하라!
 * <입력>
 * 1) 보드의 세로 크기 N, 가로 크기 M (1<= N,M <= 50)
 * N개 줄) 보드의 상태 (공백 없음) (1~9 또는 H(구멍))
 * <출력> 최대 몇 번 동전을 움직일 수 있는지. (동전을 무한번 움직일 수 있다면 -1)
 */
...
int main(void) {
    ...
    cin >> n >> m;
    memset(stat, -1, sizeof(stat)); //stat -1로 초기화 : H이면 -1
    for(int i = 1; i <= n; i++) {
        string str;
        cin >> str;
        for(int j = 1; j <= m; j++) {
