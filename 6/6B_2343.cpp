 ...
 * <입력>
 * 1) 강의의 수 N, M. (1 ≤ N ≤ 100,000 , 1 ≤ M ≤ N)
 * 2) 기타 강의의 길이. 순서대로 공백 기준. (분 단위로(자연수) <= 10000)
 ...
 * 무식하게: 모든 블루레이에 대하여 모든 강의 들어가나 체크 : O(n^2) = 1e10 = 너무 큼!!
 *  for(1~1e5) { for(1~1e5) {} } -> O(n) * O(n)
 * 이분 탐색: 블루레이 크기가 ~일 때 강의 들어가나? : O(nlogn)
 *  for(1~1e5) 부분이 이분 탐색 -> O(logn) * O(n)
 */

int len[100004]; //강의 번호에 대하여 길이
int n, m; //1 ≤ N ≤ 1e5, 1 ≤ M ≤ N <= 1e5
