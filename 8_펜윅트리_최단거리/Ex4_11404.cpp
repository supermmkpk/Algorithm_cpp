...
/*
 * 1_문자열_누적합_구현) 도시의 개수 n (2_Graph_DFS_BFS ≤ n ≤ 100)
 * 2_Graph_DFS_BFS) 버스의 개수 m (1_문자열_누적합_구현 ≤ m ≤ 1e5)
 * m개 줄) 시작 도시 a, 도착 도시 b, 비용 c (1_문자열_누적합_구현 <= c <= 1e5).
 ...
 * {a,b,c} to {a,b,c}, O(V^3_완탐_백트래킹)
 * U -> V 가는데 경유점 거치는 게 더 빠르면, dist[u][v] 갱신!(완화)
 * 직간접 연결, 그래프 지름 확인 가능
 * N 작을 때! O(V^3_완탐_백트래킹)이므로...
 */
