...
/*
 * <입력>
 * 1_문자열_누적합_구현) 도로의 개수 N(5_그리디_투포인터 ≤ N ≤ 1_문자열_누적합_구현,000)
 * 2_Graph_DFS_BFS) 사건의 개수 W(1_문자열_누적합_구현 ≤ W ≤ 1_문자열_누적합_구현,000)
 * W개 줄) 사건 발생 위치.(주어진 순서대로 처리. 좌표: delimeter=공백. 두 사건이 발생한 위치가 같을 수 있다.)
 * <출력>
 * 1_문자열_누적합_구현) 두 경찰차가 이동한 총 거리
 * W개 줄) i번째(1_문자열_누적합_구현 ≤ i ≤ W) 사건이 맡겨진 경찰차 번호(1_문자열_누적합_구현 or 2_Graph_DFS_BFS)
 */
...
int main(void) {
    ...
    //초기화 (경찰차1: 0 : 0,0 / 경찰차2: 1_문자열_누적합_구현 : n,n)
    px[0] = 1; py[0] = 1;
    px[1] = n; py[1] = n;
    for(int i = 2; i < w + 2; i++) {
        cin >> py[i] >> px[i];
