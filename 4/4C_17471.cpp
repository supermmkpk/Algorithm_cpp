 ...
 * 1) 구역의 개수 N (2 ≤ N ≤ 10)
 * 2) 구역의 인구가 1번 구역부터 N번 구역까지 순서대로 주어진다. (공백 구분) (1 ≤ 인구 수 ≤ 100)
 * N개 줄) 각 구역과 인접한 구역의 정보가 주어진다. 각 정보의 첫 번째 정수는 그 구역과 인접한 구역의 수이고, 이후 인접한 구역의 번호가 주어진다.
 * 구역 A가 구역 B와 인접하면 구역 B도 구역 A와 인접하다. 인접한 구역이 없을 수도 있다.
 * <출력> 백준시를 두 선거구로 나누었을 때, 두 선거구의 인구 차이의 최솟값. 나눌 수 없는 경우에는 -1
 */
...
int main(void) {
    ...
    cin >> n;
    for(int i = 1; i <= n; i++) { //정점 1~n 까지의 인구 수
        cin >> people[i];
    }
    //N개 줄) 각 구역과 인접한 구역의 정보가 주어진다. 각 정보의 첫 번째 정수는 그 구역과 인접한 구역의 수이고, 이후 인접한 구역의 번호가 주어진다.
    for(int i = 1; i <= n; i++) {
    ...
    for(int i = 1; i < (1 << n) - 1; i++) {
        ...
        for(int j = 0; j < n; j++) {
            if(i & (1 << j)) { //red:0, blue: 1,
                comp[j + 1] = 1; idx1 = j + 1; //blue라면 1, 마지막 블루 = idx1
            }
            else {
